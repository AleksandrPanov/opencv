 ...
 */
static inline void projectMarker(Mat& img, Ptr<Board> board, int markerIndex, Mat cameraMatrix, Mat rvec, Mat tvec,
    int markerBorder) {
    // canonical image
    Mat markerImg;
    const int markerSizePixels = 100;
    drawMarker(board->getDictionary(), board->getIds()[markerIndex], markerSizePixels, markerImg, markerBorder);
 ...
 */
static inline Mat projectBoard(Ptr<GridBoard>& board, Mat cameraMatrix, double yaw, double pitch,
    ...
    for (unsigned int index = 0; index < board->getIds().size(); index++) {
        projectMarker(img, board.staticCast<Board>(), index, cameraMatrix, rvec, tvec, markerBorder);
    }

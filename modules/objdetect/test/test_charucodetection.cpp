...
TEST(CharucoBoardGenerate, issue_24806)
{
    ...
    const float squareLength = 13.f, markerLength = 12.f;
    ...
    // TODO: fix aruco generateImage and finish test aruco corners in generated image
    /*for (const vector<Point3f>& points: board.getObjPoints()) {
        for (const Point3f& p: points) {
            Point2f arucoCorner(pixInSquare*(p.x/squareLength),
                                pixInSquare*(p.y/squareLength));
            Mat winCorner = chessboardZoneImg(Rect(Point(cvRound(arucoCorner.x) - 1, cvRound(arucoCorner.y) - 1), Size(2, 2)));
            winCorner.setTo(Scalar(127, 127, 127));
            //bool eq;
            //ASSERT_TRUE(eq);
        }
    }*/
